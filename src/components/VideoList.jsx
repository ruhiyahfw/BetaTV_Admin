...
const VideoList = () => {
  ...
  const deleteVideo = async (video, idx) => {
    const ok = window.confirm(
      'Anda yakin akan menghapus video "' + video.title + '"'
    );
    if (!ok) {
      return;
    }
  ...
  useEffect(() => {
    ...
    setVideoDisplay(
      videos ? videos.slice(idxStart, idxStart + itemCount) : null
    );
      ...
      <div className="mb-4 flex justify-between shadow-md bg-buletinLightGray sm:rounded-lg text-black">
        <div className="px-4 py-1 w-full items-center flex justify-between">
          <div className="flex gap-2">
            <p>Tampilkan</p>
            <select
              onChange={(e) => handleCountOptions(e.target.value)}
              name="itemCountOptions"
              id="itemCountOptions"
              value={itemCount}
            >
        ...
        <div className="flex shadow-md rounded-lg">
          <button
            className="active:bg-buletinDarkBlue text-white border-r-[1px] font-bold bg-buletinBlue w-[50px] rounded-tl-lg rounded-bl-lg"
            onClick={() => updatePageNumber(-1)}
          >
            {"<"}
          </button>
          <button
            className="active:bg-buletinDarkBlue text-white font-bold bg-buletinBlue w-[50px] rounded-tr-lg rounded-br-lg"
            onClick={() => updatePageNumber(1)}
          >
            {">"}
          </button>
        ...
        <table className="w-full text-sm text-left text-black table-fixed">
          ...
          <tbody>
            {videoDisplay.map((video, idx) => (
              <tr
                key={video.id}
                className="border-b border-gray-400 hover:bg-gray-300 text-center text-black"
              >
                <td>{idx + 1}</td>
                ...
                <td className="truncate text-left">
                  {" "}
                  {getCategoryVideo(video)}{" "}
                </td>
                <td className="underline text-violet-500">
                ...
                <td className="flex justify-center space-x-3 py-3">
                  ...
                  >
                    {video.loading ? (
                      ...
                      <Pen className="fill-buletinBlue" />
                    )}
                  </button>
                  <button
                    disabled={video.loading}
                    onClick={() => deleteVideo(video, idx)}
                  >
                    {video.loading ? (
                      <Loading className="fill-orange-400" />
                    ) : (
